...
write_ctt_analysis_sheet <- function(
  ...
  dist <- ctt_res$distractor_results
  opt_labels <- ctt_res$opt_labels
  n_total <- ctt_res$n_total_valid
  n_items <- ctt_res$n_items
  ...
  n_cols <- 5 + 3 * (n_opts + 1)
  data_row_start <- 4
  data_row_end <- data_row_start + n_items - 1
  avg_row <- data_row_end + 1
  ...
  for (item in 1:n_items) {
    is_unscored <- !is.null(key_vec) && length(key_vec) >= item &&
      (is.na(key_vec[item]) || key_vec[item] == "")

    if (is_unscored) {
      unscored_items <- c(unscored_items, item)
      mat[item, 2] <- "該題不予計分"
      mat[item, 3:n_cols] <- NA
      pass_rates[item] <- NA
      diff_index[item] <- NA
      disc_values[item] <- NA
      next
    ...
    mat[item, 4] <- if (!is.na(pass_rate)) sprintf("%.2f", round(pass_rate, 2)) else NA
    mat[item, 5] <- correct_key

    pass_rates[item] <- pass_rate
    diff_index[item] <- if (!is.na(hp) && !is.na(lp)) (hp + lp) / 2 else NA
    disc_values[item] <- disc_val

    cr_max_upper <- if (nrow(cr) > 0) max(cr$upper, na.rm = TRUE) else NA
    cr_max_rspP <- if (nrow(cr) > 0) max(cr$rspP, na.rm = TRUE) else NA
    ...
    for (k in 1:n_opts) {
      ...
      if (nrow(opt_row) > 0) {
        mat[item, opt_start + k - 1] <- sprintf("%.2f", round(opt_row$rspP, 2))
        mat[item, opt_start + (n_opts + 1) + k - 1] <- sprintf("%.2f", round(opt_row$upper, 2))
        mat[item, opt_start + 2 * (n_opts + 1) + k - 1] <- sprintf("%.2f", round(opt_row$lower, 2))
      }
    }
    other_row <- subset(sub, key == "9")
    if (nrow(other_row) > 0) {
      mat[item, opt_start + n_opts] <- sprintf("%.2f", round(other_row$rspP, 2))
      mat[item, opt_start + (n_opts + 1) + n_opts] <- sprintf("%.2f", round(other_row$upper, 2))
      mat[item, opt_start + 2 * (n_opts + 1) + n_opts] <- sprintf("%.2f", round(other_row$lower, 2))
    }
  }
  ...
  header2 <- c("題號", "CTT", NA, "全體", rep(NA, 6), "高分組", rep(NA, 4), "低分組", rep(NA, 4))
  openxlsx::writeData(wb, sheet_name, t(header2), startRow = 2, colNames = FALSE)

  header3 <- c(NA, "鑑別度", "難度", "通過率", "正確答案", opt_labels, "其他", opt_labels, "其他", opt_labels, "其他")
  ...
  openxlsx::mergeCells(wb, sheet_name, cols = 17:20, rows = 1)
  openxlsx::mergeCells(wb, sheet_name, cols = 2:3, rows = 2)
  openxlsx::mergeCells(wb, sheet_name, cols = 4:10, rows = 2)
  openxlsx::mergeCells(wb, sheet_name, cols = 11:15, rows = 2)
  openxlsx::mergeCells(wb, sheet_name, cols = 16:20, rows = 2)
  ...
  openxlsx::addStyle(wb, sheet_name, style2dec, rows = data_row_start:data_row_end, cols = c(2:3, 6:20), gridExpand = TRUE)

  center_style <- openxlsx::createStyle(halign = "center", wrapText = TRUE, valign = "center", border = "TopBottomLeftRight", borderStyle = "thin")
  openxlsx::addStyle(wb, sheet_name, center_style, rows = 1:3, cols = 1:n_cols, gridExpand = TRUE)
  openxlsx::addStyle(wb, sheet_name, center_style, rows = data_row_start:data_row_end, cols = c(1, 4, 5), gridExpand = TRUE)
  ...
  openxlsx::setColWidths(wb, sheet_name, cols = 1:n_cols, widths = 10)
  openxlsx::setRowHeights(wb, sheet_name, rows = 1:3, heights = 30)
  openxlsx::setRowHeights(wb, sheet_name, rows = data_row_start:data_row_end, heights = 15)
  ...
  for (item in 1:n_items) {
    ...
    for (cv in strsplit(correct_val, "、")[[1]]) {
      ...
      openxlsx::addStyle(wb, sheet_name, yellow_fill, rows = item + 3,
        cols = c(5 + opt_idx, 5 + (n_opts + 1) + opt_idx, 5 + 2 * (n_opts + 1) + opt_idx),
        gridExpand = TRUE)
    }
  }
  ...
  for (i in 1:n_items) {
    if (upper_wrong_higher[i]) {
      openxlsx::addStyle(wb, sheet_name, id_yellow, rows = i + 3, cols = 1)
    } else if (overall_wrong_higher[i]) {
      openxlsx::addStyle(wb, sheet_name, id_grey, rows = i + 3, cols = 1)
    }
  }
  ...
  for (i in 1:n_items) {
    if (!is.na(disc_values[i]) && disc_values[i] < 0.15) {
      openxlsx::addStyle(wb, sheet_name, disc_red_font, rows = i + 3, cols = 2, gridExpand = TRUE)
    }
  }
  ...
  for (item in unscored_items) {
    item_row <- item + 3
    openxlsx::mergeCells(wb, sheet_name, cols = 2:n_cols, rows = item_row)
  ...
  openxlsx::addStyle(wb, sheet_name, red_bold_style, rows = c(notes_start + 1, notes_start + 7), cols = 1)
}
